...
<div class="modern-card card p-6">
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
        <div>
            <h3 class="text-2xl font-bold text-gray-800">Kelola Transaksi</h3>
            <p class="text-gray-600">Lihat dan kelola semua transaksi peminjaman buku</p>
        </div>
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('admin.transactions.create') }}" class="action-button action-add">
    ...
    <form method="GET" action="{{ route('admin.transactions.index') }}">
        <div class="mb-6 relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari transaksi berdasarkan nama anggota atau judul buku..." class="search-input w-full py-3 pl-10 pr-4 border focus:outline-none focus:ring-2 focus:ring-soft-brown focus:border-transparent">
            @if(request('search'))
                <a href="{{ route('admin.transactions.index') }}" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="table-header">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-sm font-medium uppercase tracking-wider">Anggota</th>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="avatar mr-3">
                                {{ substr($transaction->member->nama ?? 'N', 0, 1) }}
                            </div>
                            <div>
                                <div class="font-medium text-gray-900">{{ $transaction->member->nama ?? '-' }}</div>
                                <div class="text-sm text-gray-500">{{ $transaction->member->kelas ?? '-' }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                ...
                <tr>
                    <td class="px-6 py-12 whitespace-nowrap text-center" colspan="7">
                        <div class="flex flex-col items-center">
                            ...
                            <h3 class="text-lg font-medium text-gray-900 mb-1">Belum ada data transaksi</h3>
                            <p class="text-gray-500">Mulai tambahkan transaksi peminjaman buku</p>
                            <a href="{{ route('admin.transactions.create') }}" class="mt-4 action-button action-add">
                                <i class="fas fa-plus mr-2"></i>Peminjaman Baru
                            </a>
                        </div>
                    </td>
