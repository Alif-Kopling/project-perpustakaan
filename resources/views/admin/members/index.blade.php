...
<div class="card p-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h3 class="text-lg font-semibold">Kelola Data Anggota</h3>
        <div class="flex flex-wrap gap-3">
            ...
            <a href="{{ route('admin.members.create') }}" class="btn-primary py-2 px-4 rounded transform transition-transform duration-150 hover:scale-105">
    ...
    <div id="importModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 relative">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold">Import Data Anggota</h3>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">{{ $member->nit }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ $member->kelas }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ $member->jurusan }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ $member->username }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <a href="{{ route('admin.members.edit', $member) }}" class="btn-primary py-1 px-3 rounded text-sm">
                            <i class="fas fa-edit mr-1"></i>Edit
                        </a>
                        <form action="{{ route('admin.members.destroy', $member) }}" method="POST" class="inline-block ml-2">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn-danger py-1 px-3 rounded text-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus anggota ini?')">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-center" colspan="5">
                        <div class="text-gray-500">Belum ada data anggota</div>
                    </td>
                </tr>
                @endforelse
            </tbody>
